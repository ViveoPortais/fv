...
const schema = z.object({
    ...
    licenseStateEnf: z.string().min(1, { message: "Insira um UF válido" }).trim(),
    doctorName: z.string()
    ...
    boardNumber: z.string()
        .min(1, { message: "Insira o número do conselho" }),
    professionalMobilePhone: z.string()
...
const CooperativeNurseForm = ({ optionId, bgColor }: FormProps) => {
    ...
    const createData = (form: SchemaProps) => {
        const data: IPostIncident = {
            ...
            HealthProfessionalUF: form.licenseStateEnf,
        };
        return data;
    };
    ...
    return (
        <form onSubmit={handleSubmit(onSubmit)} className="w-full space-y-4 p-4">
            ...
            <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                <div className="flex flex-col">
                    <Input {...register("nameProfessional")} placeholder="Nome do(a) enfermeiro(a)" />
                ...
                <div className="flex flex-col">
                    ...
                    {errors.professionalMobilePhone && <span className="text-red-500 text-sm">{errors.professionalMobilePhone.message}</span>}
                </div>

                <div className="flex flex-col">
                    <Input {...register("boardNumber")} placeholder="Coren" />
                    {errors.boardNumber && <span className="text-red-500 text-sm">{errors.boardNumber.message}</span>}
                </div>

                <div className="flex flex-col">
                    <Controller
                        name="licenseStateEnf"
                        control={control}
                        render={({ field }) => (
                            <CustomSelect
                                label="UF"
                    ...
                    />
                    {errors.licenseStateEnf && <span className="text-red-500 text-sm">{errors.licenseStateEnf.message}</span>}
                </div>
            </div>
...
export default CooperativeNurseForm;