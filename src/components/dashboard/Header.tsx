...
import { LuLogOut, LuFolder, LuCheckCircle } from "react-icons/lu";
...
export function Header() {
  ...
  useEffect(() => {
    if (role !== "") {
      let findRoute = routes[role]?.general.find((x) => `${x.route}` === pathname) ||
  ...
  return (
    <>
        ...
        <ul className={`flex flex-col mt-8 gap-2 w-full px-4`}>
          {role !== "" && (
            <>
              ...
              {auth.programCode && routes[role].programs[auth.programCode]?.map((programRoute) => (
                <Link
                  key={programRoute.route}
                  href={`${programRoute.route}`}
                  className={`flex gap-x-2 cursor-pointer hover:bg-zinc-100 rounded-lg p-4 ${pathname === programRoute.route &&
                    `${textColor} hover:text-zinc-800`
                    }`}
                >
          ...
          <Link
            ...
            className={`flex gap-x-2 cursor-pointer hover:bg-zinc-100 rounded-lg p-4 ${pathname === `/dashboard/program` &&
              `${textColor} hover:text-zinc-800`
              }`}
          >
      ...
      >
        <div>
          <h1 className={`text-lg lg:text-2xl font-semibold ${textColor} flex mt-5`}>
            Bem-vindo {auth.name}!
          </h1>
        </div>
        ...
        <Sheet open={isMenuOpen} onOpenChange={changeMenu}>
          ...
          <SheetContent className="m-4 rounded-lg h-auto">
            <SheetHeader>
              <SheetTitle className={`${textColor} text-xl`}>
                {auth.name}
              </SheetTitle>
            </SheetHeader>
